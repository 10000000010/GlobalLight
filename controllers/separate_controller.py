...
class SeparateMAC(BasicMAC):
    ...
    def select_actions(self, ep_batch, adj,t_ep, t_env, bs=slice(None), test_mode=False):
        # Only select actions for the selected batch elements in bs
        avail_actions = ep_batch["avail_actions"][:, t_ep]
        agent_outputs, _, _, _ = self.forward(ep_batch,adj, t_ep, test_mode=test_mode)
        chosen_actions = self.action_selector.select_action(agent_outputs[bs], avail_actions[bs], t_env,
                                                            test_mode=test_mode)
        return chosen_actions

    def forward(self, ep_batch, adjs, t, test_mode=False, t_glob=0, train_mode=False):
        agent_inputs = self._build_inputs(ep_batch, t)  # (bs*n,(obs+act+id))
        avail_actions = ep_batch["avail_actions"][:, t]
        # (bs*n,(obs+act+id)), (bs,n,hidden_size), (bs,n,latent_dim)

        agent_outs, self.hidden_states, loss_cs, diss_loss, ce_loss = self.agent.forward(agent_inputs,adjs,self.hidden_states, t=t,
    ...
    # def _build_adjs(self,batch,t):
    #     bs = batch.batch_size
    #     adjs = batch["adjs"][:, t]
    #     adjs = th.means(adjs,dim = 0,keepdim=False)
    #     # adjs = adjs.reshape(bs*self.n_agents,-1)
    #     return adjs
